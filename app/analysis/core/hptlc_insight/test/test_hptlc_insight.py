...
from ..track_inspection import TrackInspection
from ..track_sort import TrackSort
from ..pca_analysis import PCA_Analysis
from ..heatmap import Heatmap
from ..hca_analysis import HCA_Analysis

from ..file_utils import save_rgb_image

import io, os, cv2
import numpy as np
from PIL import Image
# pytestmark = pytest.mark.skip('all test still WIP')

testdata_track_detection = {"img_path": os.path.dirname(os.path.abspath(__file__))+'/test_image.png',
                            "number_of_tracks": 20,
                            "track_width": 19,
                            "bands_start": 12,
                            "front": 164
...
def test_track_inspection(track_detection):
    track_inspection = TrackInspection()
    for i in range(0, 20):
        track, densitogram = track_inspection.show_densitogram_and_signal(track_detection, i)
...
def test_track_sort(track_detection):
    ...
    buf = track_sort.sort_tracks_by_rf()
...
def test_hca_analysis(track_detection):
    reference = "1"
    num_clusters = 20
    hca = HCA_Analysis(track_detection, reference, num_clusters)
